...
static int waiter_thread_fn(void *data)
{
    ...
    while (!kthread_should_stop()) {
         ...
         *
         *   1. ISR calls complete()        → returns remaining jiffies (> 0)
         *   2. Timeout expires             → returns 0
         *   3. kthread_stop() during rmmod → the loop exits via
         *                                    kthread_should_stop()
         *
...
static int __init my_init(void) {
    ...
    waiter_thread = kthread_run(waiter_thread_fn, NULL, "comp_waiter");
