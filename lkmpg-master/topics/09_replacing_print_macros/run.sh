...
log "Running topic package for Replacing Print Macros"

do_print_string() {
    make_module print_string
    sudo insmod "$EXAMPLES_DIR/print_string.ko"
    sudo rmmod print_string
}

do_kbleds() {
    make_module kbleds
    sudo insmod "$EXAMPLES_DIR/kbleds.ko"
    sleep 1
    sudo rmmod kbleds
}

run_example "print_string" do_print_string
run_example "kbleds" do_kbleds
