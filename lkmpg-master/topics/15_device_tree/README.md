# Device Tree Study Package

## Overview
This unit packages the LKMPG chapter topic **Device Tree** into runnable or expected-output labs, backed by original repository source files and chapter references.

## Learning objectives
- Parse device-tree properties in a platform driver probe.
- Use of_match_table compatible matching.
- Understand overlay-based testing workflow and board constraints.
...
|---|---|
| `examples/devicetree.c` | Provides core implementation details for this topic unit. |
| `examples/dt-overlay.dts` | Provides core implementation details for this topic unit. |

## Example mapping
| Example | Demonstrates topic | Source file(s) | Original/adapted/reconstructed | Concept explained | Why selected | Runnable status |
|---|---|---|---|---|---|---|
| `devicetree` | Device Tree | `examples/devicetree.c` | original | of_match_table and DT property parsing | Core DT-aware driver pattern for embedded systems. | `partially_runnable` |
| `dt-overlay` | Device Tree | `examples/dt-overlay.dts` | original | runtime device-tree overlay mechanics | Connects devicetree.c probe expectations to real DT nodes. | `needs_board_support` |

## How each example relates to the topic
### `devicetree`
- Demonstrates: of_match_table and DT property parsing
- Source: `examples/devicetree.c`
- Selection reason: Core DT-aware driver pattern for embedded systems.
- Origin status: original
- Runnable status: `partially_runnable`

### `dt-overlay`
- Demonstrates: runtime device-tree overlay mechanics
- Source: `examples/dt-overlay.dts`
- Selection reason: Connects devicetree.c probe expectations to real DT nodes.
- Origin status: original
- Runnable status: `needs_board_support`
- Blocker: Overlay application is board/firmware-specific and not validated here.
...
## Output
- `devicetree` -> `outputs/devicetree.txt`
- `dt-overlay` -> `outputs/dt-overlay__expected_output.txt`
