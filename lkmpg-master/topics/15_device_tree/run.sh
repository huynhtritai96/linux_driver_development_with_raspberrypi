...
log "Running topic package for Device Tree"

do_devicetree() {
    make_module devicetree
    sudo insmod "$EXAMPLES_DIR/devicetree.ko"
    sudo rmmod devicetree
}

run_example "devicetree" do_devicetree
write_expected "dt-overlay" "Overlay application is board/firmware-specific and not validated here." "Expected flow: compile .dts -> .dtbo, apply overlay, observe devicetree probe logs."
