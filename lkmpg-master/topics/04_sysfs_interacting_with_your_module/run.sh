...
log "Running topic package for sysfs: Interacting with your module"

do_hello_sysfs() {
    make_module "hello-sysfs"
    sudo insmod "$EXAMPLES_DIR/hello-sysfs.ko"
    sudo cat /sys/kernel/mymodule/myvariable || true
    printf "456" | sudo tee /sys/kernel/mymodule/myvariable >/dev/null
    sudo cat /sys/kernel/mymodule/myvariable || true
    sudo rmmod hello-sysfs
}

run_example "hello-sysfs" do_hello_sysfs
