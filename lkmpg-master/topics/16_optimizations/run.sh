...
log "Running topic package for Optimizations"

do_static_key() {
    make_module static_key
    sudo insmod "$EXAMPLES_DIR/static_key.ko"
    sudo cat /dev/key_state || true
    printf "enable" | sudo tee /dev/key_state >/dev/null
    sudo cat /dev/key_state || true
    printf "disable" | sudo tee /dev/key_state >/dev/null
    sudo cat /dev/key_state || true
    sudo rmmod static_key
}

run_example "static_key" do_static_key
