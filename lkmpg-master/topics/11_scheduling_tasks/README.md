# Scheduling Tasks Study Package

## Overview
This unit packages the LKMPG chapter topic **Scheduling Tasks** into runnable or expected-output labs, backed by original repository source files and chapter references.

## Learning objectives
- Compare tasklets and workqueues as deferred-work mechanisms.
- Understand execution context implications.
- Handle cleanup ordering for deferred work during module exit.
...
### Documentation files
- `lkmpg.tex (Scheduling Tasks section)`
...
|---|---|
| `examples/example_tasklet.c` | Provides core implementation details for this topic unit. |
| `examples/sched.c` | Provides core implementation details for this topic unit. |

## Example mapping
| Example | Demonstrates topic | Source file(s) | Original/adapted/reconstructed | Concept explained | Why selected | Runnable status |
|---|---|---|---|---|---|---|
| `example_tasklet` | Scheduling Tasks | `examples/example_tasklet.c` | original | softirq-based deferred work | Shows non-threaded bottom-half mechanism. | `directly_runnable` |
| `sched` | Scheduling Tasks | `examples/sched.c` | original | workqueue-based deferred execution | Threaded deferred work counterpart to tasklets. | `directly_runnable` |

## How each example relates to the topic
### `example_tasklet`
- Demonstrates: softirq-based deferred work
- Source: `examples/example_tasklet.c`
- Selection reason: Shows non-threaded bottom-half mechanism.
- Origin status: original
- Runnable status: `directly_runnable`

### `sched`
- Demonstrates: workqueue-based deferred execution
- Source: `examples/sched.c`
- Selection reason: Threaded deferred work counterpart to tasklets.
- Origin status: original
- Runnable status: `directly_runnable`
...
## Output
- `example_tasklet` -> `outputs/example_tasklet.txt`
- `sched` -> `outputs/sched.txt`
