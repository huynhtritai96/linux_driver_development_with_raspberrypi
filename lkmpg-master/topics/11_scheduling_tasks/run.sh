...
log "Running topic package for Scheduling Tasks"

do_example_tasklet() {
    make_module example_tasklet
    sudo insmod "$EXAMPLES_DIR/example_tasklet.ko"
    sudo rmmod example_tasklet
}

do_sched() {
    make_module sched
    sudo insmod "$EXAMPLES_DIR/sched.ko"
    sudo rmmod sched
}

run_example "example_tasklet" do_example_tasklet
run_example "sched" do_sched
