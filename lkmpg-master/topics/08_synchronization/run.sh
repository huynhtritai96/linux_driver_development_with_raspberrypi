...
log "Running topic package for Synchronization"

do_example_mutex() {
    make_module example_mutex
    sudo insmod "$EXAMPLES_DIR/example_mutex.ko"
    sudo rmmod example_mutex
}

do_example_spinlock() {
    make_module example_spinlock
    sudo insmod "$EXAMPLES_DIR/example_spinlock.ko"
    sudo rmmod example_spinlock
}

do_example_rwlock() {
    make_module example_rwlock
    sudo insmod "$EXAMPLES_DIR/example_rwlock.ko"
    sudo rmmod example_rwlock
}

do_example_atomic() {
    make_module example_atomic
    sudo insmod "$EXAMPLES_DIR/example_atomic.ko"
    sudo rmmod example_atomic
}

run_example "example_mutex" do_example_mutex
run_example "example_spinlock" do_example_spinlock
run_example "example_rwlock" do_example_rwlock
run_example "example_atomic" do_example_atomic
