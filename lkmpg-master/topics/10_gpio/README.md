...
## Overview
This unit packages the LKMPG chapter topic **GPIO** into runnable or expected-output labs, backed by original repository source files and chapter references.

## Learning objectives
- Map GPIO lines to character device controls.
- Understand timing-sensitive GPIO protocols (DHT11).
- Identify hardware and board assumptions in kernel samples.
...
|---|---|
| `examples/dht11.c` | Provides core implementation details for this topic unit. |
| `examples/led.c` | Provides core implementation details for this topic unit. |

## Example mapping
| Example | Demonstrates topic | Source file(s) | Original/adapted/reconstructed | Concept explained | Why selected | Runnable status |
|---|---|---|---|---|---|---|
| `led` | GPIO | `examples/led.c` | original | GPIO output via char device writes | Bridges userspace controls with hardware pin state. | `needs_hardware` |
| `dht11` | GPIO | `examples/dht11.c` | original | timing-sensitive sensor protocol in kernel space | Demonstrates physical I/O assumptions and timeout handling. | `needs_hardware` |

## How each example relates to the topic
### `led`
- Demonstrates: GPIO output via char device writes
- Source: `examples/led.c`
- Selection reason: Bridges userspace controls with hardware pin state.
- Origin status: original
- Runnable status: `needs_hardware`
- Blocker: GPIO line request returns -517 on this host.

### `dht11`
- Demonstrates: timing-sensitive sensor protocol in kernel space
- Source: `examples/dht11.c`
- Selection reason: Demonstrates physical I/O assumptions and timeout handling.
- Origin status: original
- Runnable status: `needs_hardware`
- Blocker: GPIO/Sensor resources unavailable on this host.
...
## Output
- `led` -> `outputs/led__expected_output.txt`
- `dht11` -> `outputs/dht11__expected_output.txt`
