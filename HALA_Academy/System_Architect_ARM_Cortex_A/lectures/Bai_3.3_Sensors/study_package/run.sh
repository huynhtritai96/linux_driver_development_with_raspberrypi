...
run_module_example "dht11" "$EXAMPLES_DIR/dht11" "dht11.ko" "dht11" "build_only" '' 'On supported wiring, reads return measured humidity and temperature text.'
...
run_module_example "hello-sysfs" "$EXAMPLES_DIR/hello-sysfs" "hello-sysfs.ko" "hello-sysfs" "autorun" 'cat /sys/kernel/mymodule/myvariable; printf '"'"'7\n'"'"' | sudo tee /sys/kernel/mymodule/myvariable >/dev/null; cat /sys/kernel/mymodule/myvariable' 'Reading the sysfs attribute shows the current integer value and writes update it in-kernel.'
log "Finished"
